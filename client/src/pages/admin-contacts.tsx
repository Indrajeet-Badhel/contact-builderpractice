...
const getSourceUrl = (contact: ContactWithUser, type: string): string | undefined => {
...
export default function AdminContactsPage() {
  ...
  const filteredContacts = contacts.filter(contact => {
    const matchesSearch = 
      ...
      contact.userInfo.email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      contact.skills?.some(skill => skill.toLowerCase().includes(searchQuery.toLowerCase()));
    
    return matchesSearch;
  });

  // Remove duplicates based on email, githubUrl, or name similarity
  const uniqueContacts = filteredContacts.reduce((acc: ContactWithUser[], contact) => {
    const isDuplicate = acc.some(existing => {
      ...
      // Check for exact GitHub URL match
      ...
      // Check for very similar names (normalized)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Contact Detail Modal - Same as Dashboard */}
